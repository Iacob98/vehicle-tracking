-- Migration 022: Исправление циклической зависимости в RLS политиках
--
-- ПРОБЛЕМА:
-- RLS политики проверяют public.users.role, но для этого нужен доступ к users таблице!
-- Это создаёт циклическую зависимость: чтобы проверить роль, нужно прочитать users,
-- но чтобы прочитать users, нужно проверить роль!
--
-- РЕШЕНИЕ:
-- Проверять роль через JWT токен (auth.jwt()->>'user_metadata'), который доступен всегда
-- Убедиться что роль owner есть в user_metadata при логине

-- =============================================
-- HELPER FUNCTION: Получить роль из JWT
-- =============================================
...
-- =============================================
-- HELPER FUNCTION: Получить organization_id из JWT
-- =============================================
...
CREATE POLICY "car_expenses_insert" ON car_expenses FOR INSERT WITH CHECK (
  get_user_role() IN ('owner', 'admin', 'manager', 'driver')
  AND (
    get_user_role() = 'owner'
    OR organization_id = get_user_organization_id()
  )
...
-- =============================================
-- ВАЖНО: Теперь нужно убедиться что user_metadata содержит role
-- =============================================
-- Это нужно сделать при логине через trigger или обновить существующие сессии
